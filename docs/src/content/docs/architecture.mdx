...
import SlickCard from '../../components/SlickCard.astro';
...
<pre className="mermaid flex justify-center my-8 bg-transparent border-none">{`
flowchart TD
    AI[AI Coding Assistant] -->|MCP Protocol| Cortex{Cortex Server}
    
    Cortex -->|Architecture Rules| Graph[Topological Graph]
    Cortex -->|Context Lookups| Vector[Vector Engine]
    
    Graph -->|Nodes & Edges| DB1[(SQLite DB)]
    Vector -->|Markdown Embeddings| DB2[(sqlite-vec)]
...
<SlickCard title="1. The Topological Graph Engine">
...
<SlickCard title="2. The Local Vector Engine">
...
<SlickCard title="3. Cross-Engine Referencing">
